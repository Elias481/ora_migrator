...
BEGIN
   ...
   stmt := '';
   separator := '';
   FOR sch, tab, colname, pos, typ, nul, fsch, ftab, fquery IN
      EXECUTE format(
         E'SELECT schema,\n'
          '       table_name,\n'
          '       pc.column_name,\n'
      ...
      EXCEPTION
         WHEN others THEN
            ...
            GET STACKED DIAGNOSTICS
               errmsg := MESSAGE_TEXT,
               detail := PG_EXCEPTION_DETAIL;
            RAISE WARNING 'Error adjusting sequence %.%', sch, seq
               USING DETAIL = errmsg || coalesce(': ' || detail, '');
            ...
            EXECUTE
               format(
                  ...
                  sch,
                  seq,
                  format('ALTER SEQUENCE %I.%I RESTART %s', sch, seq, lastval + 1),
                  errmsg || coalesce(': ' || detail, '')
               );
